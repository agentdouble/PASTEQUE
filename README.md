...
- Bouton « Radar » dans le header: affiche les résumés journaliers (basés sur la date la plus récente disponible dans les données), hebdomadaires et mensuels générés par l’agent `looper`. Les tables visibles sont filtrées selon les droits `user_table_permissions`.
