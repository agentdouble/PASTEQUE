...
- Admin : l’onglet « Chat » configure indépendamment le contexte tickets (table + colonnes texte/titre/date + champs additionnels injectés au LLM). La colonne titre pilote l’affichage du panneau tickets du chat; persisté via `/tickets/context/config` et `/data/overview/{source}/column-roles`.
