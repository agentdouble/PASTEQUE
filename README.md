...
- Une fois connecté avec le compte administrateur, l’UI affiche l’onglet **Admin** permettant de créer de nouveaux couples utilisateur/mot de passe. L’interface a été simplifiée: **Chat**, **Explorer**, **Radar**, **Graph**, **Historique**, **Success**, **Teams** (ouvre Microsoft Teams) et **Admin** sont accessibles via des boutons dans le header (top bar). La barre de navigation secondaire a été supprimée pour éviter les doublons.
...
- Deux onglets : « Radar » (synthèse + résumé hebdomadaire par service) et « Moments forts » (exemples AQA positifs : sinistres en 48h, MyFoyer réactif, rapatriement, DocuSign, transparence dossiers).
- Sélecteur de service commun (dropdown) au-dessus des onglets : radar par service (vue unique par service, pas d’agrégat global).
