...
- Les échanges LLM (question, contexte, réponse) sont journalisés en JSON multi-lignes (pretty-printed) dans le fichier défini par `LLM_TRACE_LOG_PATH` (défaut `../logs/llm.log`), avec une entrée par appel séparée par une ligne blanche.
