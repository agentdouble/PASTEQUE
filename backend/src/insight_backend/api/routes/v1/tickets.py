...
def get_ticket_context_config(  # type: ignore[valid-type]
    ...
    service = _service(session)
    if table:
        config = service.get_config_for_table(table)
    else:
    ...
    existing = pref_repo.get_preferences_for_source(source=config.table_name)
    return TicketContextConfigResponse.from_model(
        config,
        ticket_context_fields=existing.ticket_context_fields if existing else [],
    )
